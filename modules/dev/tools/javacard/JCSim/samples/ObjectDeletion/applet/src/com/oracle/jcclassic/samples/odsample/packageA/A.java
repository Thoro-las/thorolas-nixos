 ...
 *
 * This applet used to demonstrate object deletion mechanism and also to monitor
 * memory usage as other packages/applets get added/deleted
 */

public class A extends Applet {

    public static final short ST_ERROR_BAD_TRANSIENT_MEM_DESELECT = (short) 0x9101;
    public static final short ST_ERROR_BAD_TRANSIENT_MEM_DESELECT_ELEMENT = (short) 0x9102;
    public static final short ST_ERROR_BAD_TRANSIENT_MEM_RESET = (short) 0x9103;
    public static final short ST_ERROR_BAD_TRANSIENT_MEM_RESET_ELEMENT = (short) 0x9104;
    public static final short ST_ERROR_MEM_MATCH_TRANSIENT_DESELECT_OBJECTS = (short) 0x9106;
    public static final short ST_ERROR_TRANSIENT_DESELECT_OBJECTS_EXIST = (short) 0x9107;
    public static final short ST_ERROR_TRANSIENT_RESET_OBJECTS_EXIST = (short) 0x9108;
    public static final short ST_ERROR_MEM_MATCH_TRANSIENT_RESET_OBJECTS = (short) 0x9109;
    public static final short ST_ERROR_MEM_MATCH_ALL_ATTRIBUTES = (short) 0x910A;
    public static final short ST_ERROR_MEM_MATCH_INITIAL = (short) 0x910B;
    public static final short ST_ERROR_APPLET_AID_NOT_FOUND = (short) 0x910D;
    public static final short ST_ERROR_SHAREABLE_NOT_FOUND = (short) 0x910E;
    public static final short ST_ERROR_TRANSIENT_DESELECT_MEM_NOT_RETURNED = (short) 0x910F;
    public static final short ST_ERROR_TRANSIENT_RESET_MEM_NOT_RETURNED = (short) 0x9110;
    public static final short SUCCESS = (short) 0x9000;

    // BApp1AID is the Applet AID for applet BApp1 in packageB
    static final byte[] BApp1AID = { (byte) 0xA0, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x62, (byte) 0x03,
            (byte) 0x01, (byte) 0x0C, (byte) 0x07, (byte) 0x02, (byte) 0x01 };

    short initialMemUsage; // the initial available persistent memory snapshot
    short initialTransientDeselectMem; // initial available transient memory of
    // type MEMORY_TYPE_TRANSIENT_DESELECT
    short initialTransientResetMem; // initial available transient memory of
    // type MEMORY_TYPE_TRANSIENT_RESET
    short transientDeselectUninitializedMemUsage;// memory after making array
    // but not initializing
    short transientDeselectMemUsage;// memory snapshot after making transient
    // using deselect and initializing
    short transientResetUninitializedMemUsage;// mem after making array but not
    // initializing it
    short transientResetMemUsage; // memory snapshot after making transient
    // using reset
    ...
    /**
     * method instantiates an instance of A
     */
    ...
    private void setupTrees() {
        ...
        setSleft(new ATreeNode((short) 0, MAX_TREE_DEPTH));
        setSright(new ATreeNode((short) 0, MAX_TREE_DEPTH));
        left.setData((short) 0xAA);
        right.setData((short) 0xAA);
        sleft.setData((short) 0xAA);
        sright.setData((short) 0xAA);
    }
     ...
     */
    private A() {
        // capture initial persistent memory available
        register();
    ...
    /**
     * method sets up the transient objects and captures available memory of
     * different kinds at various stages.
     */
    ...
    private short verifyTransientDeselect() {
        ...
        for (short i = 0; i < transientDeselectMem.length; i++) {
            if (transientDeselectMem[i] == null || ((TransientArrayElement) transientDeselectMem[i]).data != i) {
    ...
    private short verifyTransientReset() {
        ...
        for (short i = 0; i < transientResetMem.length; i++) {
            if (transientResetMem[i] == null || ((TransientArrayElement) transientResetMem[i]).data != i) {
    ...
    private static void setSleft(ATreeNode tree){
        sleft = tree;
    }

    private static void setSright(ATreeNode tree){
        sright = tree;
    }
    ...
    /**
     * method verifies if all memory pointed to the persistent objects was
     * collected
     */
    ...
    private void analyzeTransientDeselectMem() {
        // by this point the arrays must have had there values nulled and
        // references
        // to all transientarrayelements lost. Object deletion mechanism here
        // would have deleted
        ...
        short tempMem = JCSystem.getAvailableMemory(JCSystem.MEMORY_TYPE_PERSISTENT);
        if (tempMem != (short) (transientResetMemUsage + (transientDeselectUninitializedMemUsage - transientDeselectMemUsage))) {
            // error. not all memory collected
    ...
    /**
     * this method should be called after card was reset. It analyzes if all
     * persistent memory has been returned including the memory used by objects
     * in the transient reset array.
     */
    private void analyzeTransientResetMem() {
        ...
        short tempMem = JCSystem.getAvailableMemory(JCSystem.MEMORY_TYPE_PERSISTENT);
        if (tempMem != (short) ((transientDeselectUninitializedMemUsage - transientDeselectMemUsage) + (transientResetUninitializedMemUsage))) {
    ...
    /**
     * method sets attributes pointing to the two types of transient arrays to
     * null. This creates garbage objects to be deleted. Method also requests
     * for object deletion
     */
    ...
    /**
     * method analyzes if all transient and persistent memory have been returned
     * to the memory manager. It is called after the card has been reset.
     */
    private void analyzeRemoveAllAttributes() {
        ...
        if (initialTransientResetMem != JCSystem.getAvailableMemory(JCSystem.MEMORY_TYPE_TRANSIENT_RESET)) {
            ISOException.throwIt(ST_ERROR_TRANSIENT_RESET_MEM_NOT_RETURNED);
        }

        if (initialTransientDeselectMem != JCSystem.getAvailableMemory(JCSystem.MEMORY_TYPE_TRANSIENT_DESELECT)) {
            ISOException.throwIt(ST_ERROR_TRANSIENT_DESELECT_MEM_NOT_RETURNED);
        }
    }

    /**
     * method captures the current memory used by compareWithInitial() to check
     * if memis same.
     */
    ...
    /**
     * method compares the current memory with initialMemUsage and throws
    ...
    /**
     * method gets a shareable reference to BApp1 (Applet B instance 1) to
     * verify that we cannot delete it
     */
    private void getShareableRef() {
        ...
        // get the ref
        BApp1Ref = (JCSystem.getAppletShareableInterfaceObject(bApp1AID, (byte) 0));
        if (BApp1Ref == null) {
     ...
     * dispatches the request by calling the appropriate method and returning
     * appropriate result
     */
    @Override
